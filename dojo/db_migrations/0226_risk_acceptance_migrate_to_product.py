...
def migrate_risk_acceptance_to_product(apps, schema_editor):
    ...
    for risk_acceptance in Risk_Acceptance.objects.filter(product__isnull=True):
        ...
        if engagement:
            ...
            # we need to handle it. We could either delete it or skip it.
            # For now, let's skip it and let the admin handle orphaned records.
            risk_acceptances_orphaned += 1
            print(f"Warning: Risk Acceptance {risk_acceptance.id} '{risk_acceptance.name}' has no associated engagement")

    print(f"Migration complete: {risk_acceptances_updated} risk acceptances migrated to product level")
    if risk_acceptances_orphaned > 0:
        print(f"Warning: {risk_acceptances_orphaned} orphaned risk acceptances found (no associated engagement)")
...
def reverse_migrate_risk_acceptance_to_engagement(apps, schema_editor):
    ...
    for risk_acceptance in Risk_Acceptance.objects.filter(product__isnull=False):
        ...
        if engagement:
            ...
            print(f"Warning: Could not find engagement for Risk Acceptance {risk_acceptance.id} in product {risk_acceptance.product.name}")

    print(f"Reverse migration complete: {risk_acceptances_restored} risk acceptances restored to engagement level")


class Migration(migrations.Migration):

    dependencies = [
        ('dojo', '0225_risk_acceptance_add_product_field'),
    ]
